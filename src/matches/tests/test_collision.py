...
class GridCollisionTests(TestCase):
    ...
    def test_scv_vs_base_sliding(self):
        """scv vs Base: Verify clean sliding around the 4x3 footprint."""
        ...
        # scv moving from left to right, straight through the center
        scv = Entity('scv', 'p1', 8, 11.5)
        scv.action = MoveAction(Vector2D(16, 11.5))
        ...
        # scv should have moved past the base (target is at 16)
        # Speed is 1.0, so 40 ticks is plenty even with sliding
        self.assertGreater(scv.pos.x, 14) 
        # Check against static grid only to avoid self-blocking in test verification
        self.assertFalse(gs.nav_grid.is_blocked(scv.pos, check_dynamic=False))

    def test_scv_vs_arc(self):
        """scv vs Arc: Verify the unit follows the curve of a mineral arc instead of getting stuck."""
        gs = self.MockGameState(self.width, self.height)
        ...
        # scv inside the concavity
        scv = Entity('scv', 'p1', 13, 13)
        # Target is past the corner (bottom-right)
        scv.action = MoveAction(Vector2D(20, 20))
        gs._spawn_entity(scv)
        ...
        self.assertGreater(scv.pos.x, 16)
        self.assertLessEqual(scv.pos.y, 20.1) # Small tolerance for diagonal movement
    ...
    def test_the_square_trap(self):
        ...
        # But leave small gaps at corners for scvs to potentially escape
        ...
        # scv trapped inside at center (12.5, 12.5)
        scv = Entity('scv', 'p1', 12.5, 12.5)
        scv.action = MoveAction(Vector2D(50, 50))
        gs._spawn_entity(scv)

        initial_pos = scv.pos.copy()
        ...
        for tick in range(100):
            gs.nav_grid.clear_dynamic()
            gs.nav_grid.set_dynamic(scv.pos.x, scv.pos.y, scv.id)
            scv.update(gs)
            
            if scv.pos.dist_to(initial_pos) > 1:
                moved = True
                dx = scv.pos.x - initial_pos.x
                dy = scv.pos.y - initial_pos.y
                if abs(dx) > abs(dy):
        ...
        # scv should have tried different directions (at least attempted movement)
        self.assertTrue(moved, f"scv should try to move. Pos: {scv.pos}")
        
        # In a sealed trap, scv cannot escape without pathfinding
